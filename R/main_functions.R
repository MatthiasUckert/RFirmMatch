...
match_name <- function(.table_firm, .table_match, .col_match) {
  ...
  itab <- dplyr::inner_join(
    ...
    dplyr::mutate(
      sim = stringdist::stringsim(name_firm, name_match),
...
match_substring <- function(.table_firm, .table_match, .min_char = 0.25, .progress = TRUE) {
  name_adj <- name_adj1 <- name_firm <- tmp <- NULL
  ...
  for (i in seq) {
    ...
    itab_matches <- match_name(itab_firm, itab_match, tmp)
