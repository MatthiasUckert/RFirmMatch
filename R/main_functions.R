...
#' @param .col_match The Matching Column
#' @param .legal_form Does the Dataset contains legal forms? (Default = TRUE)
#'
#' @return
...
match_name <- function(.table_firm, .table_match, .col_match, .legal_form = TRUE) {
  id_firm <- id_match <- country_code_firm <- country_code_match <-
    ...
    n_id_match <- name_firm <- name_match <- name_adj <- legal_form_id_firm <-
    name_adj_firm <- name_adj_match <- sim <- sim_adj <- NULL

  itab_firm <- dplyr::mutate(.table_firm, match = {{ .col_match }})
  itab_match <- dplyr::mutate(.table_match, match = {{ .col_match }})
  ...
  if (.legal_form) {
    itab <- itab %>%
      dplyr::mutate(
        ...
        sim_adj = stringdist::stringsim(name_adj_firm, name_adj_match)
      ) %>%
      dplyr::select(
        id_firm, id_match,
        match, name_firm, name_match, name_adj_firm, name_adj_match,
        country_code_firm, country_code_match, legal_form_firm, legal_form_match,
        same_country, same_legal_form, n_id_firm, n_id_match, sim, sim_adj
      )
  } else {
    itab <- itab %>%
      dplyr::select(
        ...
        same_country, n_id_firm, n_id_match, sim
      )
  }
...
#'  id = 1:2,
#'  country_code = c("DEU", NA),
#'   name1 = c("BASFss AG", "BASFss SE"),
#'   name2 = c("BASFss Germany AG", "BASFss Germany SE")
#' )
#' table_match <- tibble::tibble(
#'  id = 1:2,
#'  country_code = c("DEU", "DEU"),
...
match_substring <- function(.table_firm, .table_match, .min_char = 0.25, .progress = TRUE, .legal_form = TRUE) {
  name_adj <- name_firm <- tmp <- name <- NULL



  if (.legal_form) {
    int_max = max(nchar(.table_firm$name_adj))
    seq <- seq.int(int_max, 3, by = - 2)

    itab_firm <- dplyr::mutate(.table_firm, tmp = name_adj)
    itab_match <- dplyr::mutate(.table_match, tmp = name_adj)
  } else {
    int_max = max(nchar(.table_firm$name))
    seq <- seq.int(int_max, 3, by = - 2)

    itab_firm <- dplyr::mutate(.table_firm, tmp = name)
    itab_match <- dplyr::mutate(.table_match, tmp = name)
  }
  ...
  for (i in seq) {
    itab_firm <- dplyr::mutate(itab_firm, tmp = stringi::stri_sub(tmp, 1, i))
    itab_match <- dplyr::mutate(itab_match, tmp = stringi::stri_sub(tmp, 1, i))

    itab_matches <- match_name(itab_firm, itab_match, tmp, .legal_form)

    itab_firm  <- adjust_matches(itab_matches, itab_firm, tmp)
    itab_match <- adjust_matches(itab_matches, itab_match, tmp)
...
#' match1 <- match_name(table_firm, table_match, name)
#' match2 <- match_name(table_firm, table_match, name_adj)
match_approximate <- function(
  .table_firm, .table_match, .col_match, .legal_form = TRUE,
  .max_dist = 2, .method = c("osa", "lv", "dl", "hamming", "lcs", "qgram", "cosine", "jaccard", "jw", "soundex"),
  .ignore_case = FALSE) {
  id_firm <- id_match <- country_code_firm <- country_code_match <-
    country_code_match <- legal_form_id_match <- name <- legal_form_firm <-
    legal_form_match <- same_country <- same_legal_form <- n_id_firm <-
  ...
  itab <- fuzzyjoin::stringdist_inner_join(
    x = itab_firm,
    y = itab_match,
    by = "match",
    max_dist = .max_dist,
  ...
  colnames(itab) <- gsub(".x", "_firm", colnames(itab), fixed = TRUE)
  colnames(itab) <- gsub(".y", "_match", colnames(itab), fixed = TRUE)
  ...
  if (.legal_form) {
    itab <- itab %>%
      ...
      dplyr::select(
        id_firm, id_match,
        name_firm, name_match, name_adj_firm, name_adj_match,
        country_code_firm, country_code_match, legal_form_firm, legal_form_match,
        same_country, same_legal_form, n_id_firm, n_id_match, sim, sim_adj
      )
  } else {
...
#' @return A Dataframe
#' @export
adjust_matches <- function(.tab_matches, .tab, .col_match) {
  name <- id <- NULL

  ids <- .tab %>%
    dplyr::filter({{ .col_match }} %in% .tab_matches[["match"]]) %>%
    dplyr::pull(id)
